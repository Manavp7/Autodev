...
import { 
  ...
  Search as SearchIcon, Command, IndianRupee, DollarSign, Euro, Lock
} from 'lucide-react';
      ...
      <div className="flex items-center gap-4">
              ...
              <div className="max-h-96 overflow-auto custom-scrollbar">
                {notifications.map((notif) => (
                  <div 
                    key={notif.id} 
                    className={cn(
                      "p-4 border-b border-border-dark hover:bg-primary transition-colors cursor-pointer group",
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <span className="text-[9px] text-text-secondary font-medium">{notif.time}</span>
                    </div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex flex-col items-end hidden md:flex">
            ...
            <Badge variant="accent" className="mt-1">{user?.role} - {user?.designation}</Badge>
          </div>
          <button 
            onClick={logout}
