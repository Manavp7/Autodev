...
export default function ShortageManagement() {
  ...
  const handleExpedite = () => {
    ...
    if (idx !== -1) {
      ...
      // Handoff #8: shortage.raised -> AutoMFG PlanningPage flashes impacted line
      const s = updated[idx];
      emit({
        type: 'shortage.raised',
        partNumber: s.partNumber,
        gap: Math.max(0, (s.requiredQty ?? 0) - (s.availableStock ?? 0)),
        impactLines: s.impactedLines ?? [s.supplier ?? 'Line Alpha'],
      });
    }
                      ...
                      {shortage.status !== 'Expedited' && (
                        <button 
                          onClick={() => {
                            ...
                            // Handoff #8 alt-path: PR-trigger also flashes Planning
                            emit({
                              type: 'shortage.raised',
                              partNumber: shortage.partNumber,
                              gap: Math.max(0, (shortage.requiredQty ?? 0) - (shortage.availableStock ?? 0)),
                              impactLines: shortage.impactedLines ?? ['Line Alpha'],
                            });
                          }}
