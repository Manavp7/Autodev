...
export function PlanningPage() {
  ...
  // Cross-module banners surfaced from the event bus.
  const [bomBanner, setBomBanner] = useState<string | null>(null)
  const [shortageFlash, setShortageFlash] = useState<string | null>(null)

  const week = WEEKS[weekIdx]

  // Handoff #1: BOM released by AutoDev BOMHub -> show banner here
  useEventBusEffect('bom.released', (e) => {
    setBomBanner(`MBOM ${e.mbomId} released — ${e.partNumbers.length} parts ready for planning.`)
    toast(`MBOM ${e.mbomId} released to Production Planning`, 'success')
  })

  // Handoff #8: Shortage raised by AutoSCM -> flash impacted line
  useEventBusEffect('shortage.raised', (e) => {
    setShortageFlash(`Shortage ${e.partNumber} impacts ${e.impactLines.join(', ')} (gap ${e.gap}).`)
    toast(`Shortage on ${e.partNumber} — adjust schedule`, 'error')
    // auto-clear after 8s
    window.setTimeout(() => setShortageFlash(null), 8000)
  })

  // Auto-clear BOM banner after 10s
  useEffect(() => {
    if (!bomBanner) return
    const t = window.setTimeout(() => setBomBanner(null), 10000)
  ...
  const handleCreate = () => {
    ...
    // Handoff #5: Plan approved -> WO auto-creation
    emit({ type: 'plan.approved', planId, lineId: form.line })
  ...
  return (
    <div className="flex flex-col gap-5 h-full">
      ...
      {bomBanner && (
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-emerald-800 text-body-sm flex items-center justify-between">
          <span>{bomBanner}</span>
          <button onClick={() => setBomBanner(null)} className="text-emerald-700 hover:underline text-body-xs font-bold">Dismiss</button>
        </div>
      )}
      {shortageFlash && (
        <div className="rounded-lg border border-red-300 bg-red-50 px-4 py-3 text-red-800 text-body-sm flex items-center justify-between animate-pulse">
          <span><AlertTriangle size={14} className="inline mr-2"/>{shortageFlash}</span>
          <button onClick={() => setShortageFlash(null)} className="text-red-700 hover:underline text-body-xs font-bold">Dismiss</button>
        </div>
      )}
