// AutoSCM AppContext.
// Auth + notifications now flow from the shared zustand stores under
// ../stores. This context owns SCM-specific state: theme, currency, FX,
// records, and the global detail drawer.
...
export function AppProvider({ children }) {
  ...
  // Note: previously this was `useState(... || SYSTEM_USERS[0])` which silently
  // logged in the admin every time. We now strictly hydrate from localStorage,
  // and otherwise rely on the shared auth store.
  ...
  return (
    <AppContext.Provider
      value={{
        // auth / permissions (from shared store)
