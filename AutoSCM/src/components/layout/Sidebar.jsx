...
import {
  ...
  FileSignature, ChevronLeft, ChevronRight, Settings, LogOut, HelpCircle
} from 'lucide-react';
...
export default function Sidebar({ isOpen, setIsOpen }) {
  ...
  // Shortage alert dot is now data-driven instead of always-on.
  const { records } = useApp();
  const shortageCount = (records?.prs ?? []).filter((r) => r.priority === 'Critical' || r.priority === 'High').length;
  ...
  return (
    <div className={cn(
      "flex flex-col bg-sidebar border-r border-border-dark transition-all duration-300 z-20 shrink-0",
      isOpen ? "w-64" : "w-20"
    )}>
      {/* Logo Area */}
      ...
      <nav className="flex-1 py-4 overflow-y-auto space-y-1 px-2 custom-scrollbar">
        ...
        {navItems.map((item) => (
          ...
          >
            <item.icon className={cn("w-5 h-5 shrink-0", item.alert && shortageCount > 0 && "text-warning")} />
            {isOpen && (
              <span className="truncate text-[13px] tracking-tight">{item.name}</span>
            )}
            {item.alert && shortageCount > 0 && (
              <span className="absolute right-3 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-warning shadow-[0_0_8px_rgb(var(--warning)/0.5)]" />
            )}
      ...
      {/* Bottom Area */}
      <div className="p-3 border-t border-border-dark shrink-0 space-y-1">
        <div className={cn("flex items-center gap-3 px-2 py-2", !isOpen && "justify-center")}>
          <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-accent to-blue-700 shrink-0 border-2 border-surface flex items-center justify-center text-white font-bold text-xs">
            {user?.avatarInitials || '??'}
          </div>
          {isOpen && (
            <div className="flex flex-col overflow-hidden">
              <span className="text-sm font-medium text-text-primary truncate">{user?.name ?? 'Guest'}</span>
              <span className="text-[10px] text-text-secondary truncate capitalize">
                {user?.designation ?? user?.role?.replace(/_/g, ' ').toLowerCase() ?? '—'}
              </span>
            </div>
          )}
        </div>
        ...
        <NavLink
          ...
          className={cn(
            'flex items-center gap-3 px-3 py-2 rounded-lg text-text-secondary hover:text-text-primary hover:bg-surface transition-all',
            !isOpen && 'justify-center'
          )}
        >
          <HelpCircle className="w-4 h-4 shrink-0" />
          {isOpen && <span className="text-[13px]">Help</span>}
        </NavLink>
        ...
        <button
          ...
          className={cn(
            'w-full flex items-center gap-3 px-3 py-2 rounded-lg text-danger hover:bg-danger/10 transition-all',
            !isOpen && 'justify-center'
          )}
        >
          <LogOut className="w-4 h-4 shrink-0" />
          {isOpen && <span className="text-[13px] font-semibold">Logout</span>}
        </button>
      </div>
    </div>
  );
