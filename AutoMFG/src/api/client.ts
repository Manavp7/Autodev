...
api.interceptors.response.use(
  ...
  (err) => {
    if (err.response?.status === 401) {
      ...
      // Let the router handle redirection so cache stays warm.
      window.dispatchEvent(new CustomEvent('automfg:unauthorized'))
    }
...
// Auth — match by username OR email; map demo accounts to seeded user records.
const usernameToEmailMap: Record<string, string> = {
  manager:  'manager@automfg.local',
  lead:     'lead@automfg.local',
  operator: 'operator@automfg.local',
  plantmgr: 'plant@automfg.local',
  leader:   'leader@automfg.local',
}

mock.onPost('/auth/login').reply(config => {
  const body = JSON.parse(config.data) as { email?: string; username?: string }
  const key = (body.email || body.username || '').toLowerCase()
  const email = usernameToEmailMap[key] ?? body.email ?? ''
  const user =
    db.users.find(u => u.email === email) ||
    db.users.find(u => (u as any).username?.toLowerCase() === key) ||
